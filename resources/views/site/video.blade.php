                            ...
                            <div class="post_wrapper">
                                <div class="post_header">
                                    ...
                                    <ul class="post_author">
                                        <li>Автор : <a
                                                    href="{{url('/articles?author='.$video->author)}}">{{$video->author}}</a>
                                        </li>
                        ...
                        @if( Auth::user())
                            <div class="comment_respond">
                                ...
                                <form class="comment__form" action="{{route('comment')}}" method="GET">
                                    <div>
                                        @if(Auth::user()->getRatingForVideo($video->id))
                                            <div class="user-rate-rating">
                                                <p>вы уже прогололсовали</p>
                                                <label>
                                                    <span class="rate-icon @if(Auth::user()->getRatingForVideo($video->id) && Auth::user()->getRatingForVideo($video->id) >= 1 ) active @endif">★</span>
                                                    <span class="rate-icon @if(Auth::user()->getRatingForVideo($video->id) && Auth::user()->getRatingForVideo($video->id) >= 2 ) active @endif">★</span>
                                                    <span class="rate-icon @if(Auth::user()->getRatingForVideo($video->id) && Auth::user()->getRatingForVideo($video->id) >= 3 ) active @endif">★</span>
                                                    <span class="rate-icon @if(Auth::user()->getRatingForVideo($video->id) && Auth::user()->getRatingForVideo($video->id) >= 4 ) active @endif">★</span>
                                                    <span class="rate-icon @if(Auth::user()->getRatingForVideo($video->id) && Auth::user()->getRatingForVideo($video->id) >= 5 ) active @endif">★</span>
                                                </label>
                                            </div>
                                        @else
