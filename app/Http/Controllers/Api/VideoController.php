    ...
    public function check_vip_code(Request $request)
    {
        ...
        try {
            if ($video = Video::query()->find($request->id)) {
                if ($code = VipCode::query()->where(['object_id' => $request->id, 'object_type' => VipCode::VIDEO_TYPE, 'code' => strtoupper($request->code)])
                    ->where('try_count', '>', 0)->first()) {
                    $code->decrement('try_count');
                    $code->save();
