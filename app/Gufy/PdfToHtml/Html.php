...
class Html extends Dom
{
    ...
    public static function getStyleAndTextInElement(Dom\HtmlNode $element, $need_style = 'left'){
        if($element instanceof Dom\HtmlNode){
            $styles = $element->getAttribute('style');
            $styles_arr = explode(';',$styles);
            foreach ($styles_arr as $style){
                $style_arr = explode(':', $style);
                if(isset($style_arr[0]) && $style_arr[0] == $need_style){
                    return intval($style_arr[1]);
                }
            }
        }
        return null;
